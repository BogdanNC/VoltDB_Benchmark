...
CREATE TABLE male_document_ids (
    id BIGINT PRIMARY KEY 
);
...
INSERT INTO male_document_ids (id)
SELECT d.id AS id
FROM documents d
INNER JOIN documents_authors da ON d.id = da.id_document
INNER JOIN authors a ON da.id_author = a.id
...
FROM vocabulary v 
INNER JOIN male_document_ids fdi ON v.id_document = fdi.id
GROUP BY v.id_word;
...
FROM (
    SELECT d.id as id, w.word AS word,
           (v.tf * (1 + LOG((SELECT COUNT(id) FROM male_document_ids) / q_wcd.wordCountDocs)) * (1.6 + 1)) /
